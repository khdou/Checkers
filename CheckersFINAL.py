    ...
    if not canvas.data.splash and not canvas.data.gameOver:
        ...
        if isLegalMovePressed(rowi,coli): #checks if piece is the player
            ...
            if player > 0:
                ...
                player = "Red"
            tkMessageBox.showwarning("Error", "Please click on your own piece. \n You are: %s" % (player))   
...
def drawSideBox():
    ...
    for piece in xrange(blackCaptured):
        ...
        player = -1
        drawCaptured(startx,starty,p,player,size)
    canvas.create_text(630,150,text="Captured : %d" % (blackCaptured),font=("Papyrus",20,"bold"),anchor=W)
    canvas.create_text(630,450,text="Captured : %d" % (redCaptured), font=("Papyrus",20,"bold"),anchor=W,fill="red")
    canvas.create_text(625,370,text="Press \"R\" to Restart",font=("Papyrus",15,"bold"),anchor=W,fill="saddlebrown")
    canvas.create_text(625,400,text="Or \"M\" for the Main Menu",font=("Papyrus",15,"bold"),anchor=W,fill="saddlebrown")
    player = canvas.data.player
    if canvas.data.gameOver:
        if player == -1:
            ...
            color = "Red"
        canvas.create_text(640,240,text="Game Over!",font=("Papyrus",25,"bold"),anchor=W,fill=color)
        canvas.create_text(640,290,text="%s Wins!" % (color),font=("Papyrus",25,"bold"),anchor=W,fill=color)
    elif player == -1:
        canvas.create_rectangle(620,270,880,330,fill="",outline="red",width=5)
        canvas.create_text(640,300,text="Player 1's turn",font=("Papyrus",25,"bold"),anchor=W,fill="red")
    elif player == 1:
        canvas.create_rectangle(620,270,880,330,fill="",outline="black",width=5)
        canvas.create_text(640,300,text="Player 2's turn",font=("Papyrus",25,"bold"),anchor=W,fill="black")
    if canvas.data.onePlayer and canvas.data.AI:
        canvas.create_text(640,230,text="AI is thinking...",font=("Papyrus",25,"bold"),anchor=W,fill="black")
...
def loadSplashScreen():
    ...
    canvas.create_text(width/2,height*.3,text="Checkers",fill="khaki2",font=("Papyrus",55,"bold"))
    canvas.create_text(width/2,height*.42,text="By Kevin Dou",fill="khaki2",font=("Papyrus",30,"bold"))
    b1 = Button(canvas,text="Single Player",command=button1Pressed,activebackground="saddlebrown",\
        activeforeground="white",bg="khaki",fg="saddlebrown",height=1,width=10,font=("Papyrus",20,"bold"),overrelief="sunken")
    ...
    sx = 240
    sy = 100
    for row in xrange(42):
        if row % 2 == 0:
            canvas.create_rectangle(cs*row,0,cs*(row+1),cs,fill="saddlebrown",width=0)
            canvas.create_rectangle(cs*row,height-cs,cs*(row+1),height,fill="khaki2",width=0)
            canvas.create_rectangle(c*row+sx,sy,c*(row+1)+sx,sy+c,fill="saddlebrown",width=0)
            canvas.create_rectangle(c*row+sx,sy+200,c*(row+1)+sx,sy+c+200,fill="saddlebrown",width=0)
        elif row % 2 == 1:
            canvas.create_rectangle(cs*row,0,cs*(row+1),cs,fill="khaki2",width=0)
            canvas.create_rectangle(cs*row,height-cs,cs*(row+1),height,fill="saddlebrown",width=0)
            canvas.create_rectangle(c*row+sx,sy,c*(row+1)+sx,sy+c,fill="khaki2",width=0)
            canvas.create_rectangle(c*row+sx,sy+200,c*(row+1)+sx,sy+c+200,fill="khaki2",width=0)
    for col in xrange(21):
        if col % 2 == 0:
            canvas.create_rectangle(0,cs*col,cs,cs*(col+1),fill="saddlebrown",width=0)
            canvas.create_rectangle(width-cs,cs*col,width,cs*(col+1),fill="khaki2",width=0)
            canvas.create_rectangle(sx,c*col+sy,sx+c,c*(col+1)+sy,fill="saddlebrown",width=0)
            canvas.create_rectangle(sx+420,c*col+sy,sx+c+420,c*(col+1)+sy,fill="saddlebrown",width=0)
        elif col % 2 == 1:
            canvas.create_rectangle(0,cs*col,cs,cs*(col+1),fill="khaki",width=0)
            canvas.create_rectangle(width-cs,cs*col,width,cs*(col+1),fill="saddlebrown",width=0)
            canvas.create_rectangle(sx,c*col+sy,sx+c,c*(col+1)+sy,fill="khaki2",width=0)
            canvas.create_rectangle(sx+420,c*col+sy,sx+c+420,c*(col+1)+sy,fill="khaki2",width=0)
...
def drawAIButtons():
    ...
    b3 = Button(text="Beginner",command=button3Pressed,activebackground="saddlebrown",\
        activeforeground="white",bg="khaki",fg="saddlebrown",height=1,width=10,font=("Papyrus",15,"bold"),overrelief="sunken")
    canvas.create_window(width*.15, height*.8, window=b3)
    b4 = Button(text="Intermediate",command=button4Pressed,activebackground="saddlebrown",\
        activeforeground="white",bg="khaki",fg="saddlebrown",height=1,width=10,font=("Papyrus",15,"bold"),overrelief="sunken")
    canvas.create_window(width*.35, height*.8, window=b4)
    b5 = Button(text="Advanced",command=button5Pressed,activebackground="saddlebrown",\
        activeforeground="white",bg="khaki",fg="saddlebrown",height=1,width=10,font=("Papyrus",15,"bold"),overrelief="sunken")
    canvas.create_window(width*.55, height*.8, window=b5)
"""
